...
$pdf->Cell(31,7,utf8_decode("(Km) Inicial"),1,0,"c");
$pdf->Cell(31,7,utf8_decode("(KM) Final"),1,0,"c");
...
$pdf->Cell(55,7,"Ferramentas",1,0,"c");
...
while ($row = mysqli_fetch_array($execute_fer)) {
    $pdf->SetFont("Arial","",10);
    $pdf->cell(55,7,utf8_decode($row["fer_descricao"]),1,0,"c");
...
$pdf->SetX("102");
$pdf->SetFont("Arial","b",11);
$pdf->Cell(55,7,"Epi",1,0,"c");
$pdf->Cell(35,7,utf8_decode("Condição"),1,0,"c");
$pdf->Ln();
$epi = "SELECT fer_descricao, fer_has_transacao_descricao, fer_direcionamento FROM transacao 
INNER JOIN ferramentas_has_transacao ON transacao_tra_id = tra_id INNER JOIN ferramentas ON fer_id = ferramentas_fer_id WHERE tra_id = '$id' AND fer_direcionamento = 'Epi'";
$execute_epi = mysqli_query($conexao, $epi);
while ($linha = mysqli_fetch_array($execute_epi)) {
    $pdf->SetFont("Arial","",10);
    $pdf->SetX("102");
    $pdf->cell(55,7,utf8_decode($linha["fer_descricao"]),1,0,"c");
    $pdf->Cell(35,7,utf8_decode($linha["fer_has_transacao_descricao"]),1,0,"c");
    $pdf->Ln(); 
    }
    $pdf->SetY("165");
    $pdf->Ln();   
    $pdf->SetFont("Arial","b",11);
     $pdf->SetY("300");
    $pdf->Cell(55,7,utf8_decode("Situação do Veículo"),1,0,"c");
    $pdf->Cell(27,7,utf8_decode("Condição"),1,0,"c");
    $pdf->Ln();
    ...
    while ($linha = mysqli_fetch_array($execute_pecas)) {
        $pdf->SetFont("Arial","",10);
        $pdf->cell(55,7,utf8_decode($linha["pe_descricao"]),1,0,"c");
        $pdf->Cell(27,7,utf8_decode($linha["tran_pe_condicao"]),1,0,"c");
        $pdf->Ln(); 
        }
        $pdf->Ln();
        $sql = mysqli_query($conexao, $query);
