...
<!-- pagination -->
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  <h4 class="card-title mt-0">Relatórios de Sobral</h4>
                </div>
                <div class="card-body">
                  <div class="table-responsive">
                  <table class="table">
                      ...
                      <tbody>
                        <?php
                        include "../../../controle/conexao.php";
                        $query_sobral = "SELECT usu_nome,tra_data, tra_id, tra_tipo,tra_quilometragem, tra_quilometragemf, auto_modelo, auto_placa, tra_obs FROM transacao INNER JOIN usuarios ON usuarios_usu_id = usu_id 
                        INNER JOIN automoveis ON automoveis_auto_id = auto_id WHERE usu_cidade = 'Sobral' ORDER BY `transacao`.`tra_data` DESC";
                      ...
                      // Quantidade de registros por página
                                ...
                                $inicio2 = $pc2 - 1;
                                $inicio2 = $inicio2 * $registros2;
                                $limite2 = mysqli_query($conexao, "$query_sobral LIMIT $inicio2,$registros2");
                                $todos2 = mysqli_query($conexao, "$query_sobral");
                                $tr2 = mysqli_num_rows($todos);
                                $total_paginas2 = ceil($tr2 / $registros2);
                                $anterior2 = $pc2 - 1;
                                $proximo2 = $pc2 + 1;
...
// ------------
                        ...
                        $sql_sobral = mysqli_query($conexao, $query_sobral);
                        if(($query_sobral) AND ($sql_sobral->num_rows != 0 )){
                        while ($dados = mysqli_fetch_assoc($limite2)) {
                          $id = $dados['tra_id'];
                          $nome = $dados['usu_nome'];
                          $tipo = $dados['tra_tipo'];
                          $automovel = $dados['auto_modelo'];
              ...
              <td class='text-center'>$placa</td>
                            ...
                            <td class='text-center'>$obs</td>
							<td class='text-center'><a href='relatorios/ver_relatorio_$aux.php?id=$id' target='black'><i class='fa fa-eye' aria-hidden='true'></i></a></td>
							<!--<td class='text-center'><a href='#' onClick='apagar_relatorio($id)'><i class='fa fa-trash' aria-hidden='true'></i></a></td></tr>-->
						";
                        }
                        }else{
                          echo "<tr><th>Nenhum relatório feito</th></tr>";
                    ...
                    <center>
                        <ul class="pagination mx-auto">
                            ...
                            $limite2 = $pagina + $links_laterais;*/
                             ...
                             for ($i = $pagina2 - 3 , $limiteDeLinks2 = $i + 6; $i <= $limiteDeLinks2 ; $i++) {
                                 if($i < 1){
                                    $i = 1;
                                    $limiteDeLinks2 = 7;}
